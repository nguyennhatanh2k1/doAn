...
export default function ProductModalAdd(props) {
    const { isOpen, closeModal, onConfirm, listCategory,listBrand,listColor,listSize } = props;
    ...
    const changeImage = ({ fileList: newFileList, file }) => {
      // Kiểm tra nếu file.status là "done", có nghĩa là upload đã thành công
      if (file.status === 'done') {
        // Lấy link của ảnh từ response của server
        const imageUrl = file.response.url;
  
        // Lưu lại link của ảnh vào state hoặc nơi bạn muốn
